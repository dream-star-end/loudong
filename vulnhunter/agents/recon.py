...
Uses SecureHttpClient for all requests (ScopeGuard + RateLimiter + Audit).
Optionally uses LLM to analyze fingerprints.
"""
...
from vulnhunter.tools.scope_guard import ScopeViolationError

logger = logging.getLogger(__name__)

SECURITY_HEADERS = {
    "strict-transport-security": {
        "wstg": "WSTG-CONF-07", "title": "Missing HTTP Strict-Transport-Security Header", "severity": "medium",
    },
    "content-security-policy": {
        "wstg": "WSTG-CONF-12", "title": "Missing Content-Security-Policy Header", "severity": "medium",
    },
    "x-frame-options": {
        "wstg": "WSTG-CONF-07", "title": "Missing X-Frame-Options Header", "severity": "low",
    },
    "x-content-type-options": {
        "wstg": "WSTG-CONF-07", "title": "Missing X-Content-Type-Options Header", "severity": "low",
    },
    "x-xss-protection": {
        "wstg": "WSTG-CONF-07", "title": "Missing X-XSS-Protection Header", "severity": "info",
    },
    "referrer-policy": {
        "wstg": "WSTG-CONF-07", "title": "Missing Referrer-Policy Header", "severity": "info",
    },
    "permissions-policy": {
        "wstg": "WSTG-CONF-07", "title": "Missing Permissions-Policy Header", "severity": "info",
    },
}
            ...
            for host in hosts:
                ...
                for hdr, info in SECURITY_HEADERS.items():
                    if hdr not in headers_lower:
                        findings.append({
                            "title": info["title"], "category": "Security Misconfiguration",
                            "wstg_refs": [info["wstg"]], "severity": info["severity"],
                            "confidence": 0.95,
                ...
                if fingerprints:
                    ...
                    if "server" in headers_lower:
                        sv = headers_lower["server"].lower()
                        if any(v in sv for v in ["apache/", "nginx/", "iis/"]):
                            findings.append({
                                "title": "Server Version Disclosure", "category": "Information Disclosure",
                                "wstg_refs": ["WSTG-INFO-02"], "severity": "low", "confidence": 0.98,
                    ...
                    try:
                        ...
                        if any(kw in body for kw in ["traceback", "stack trace", "exception", "debug", "at line"]):
                            findings.append({
                                ...
                                "category": "Error Handling", "wstg_refs": ["WSTG-ERRH-01"],
                ...
                if self.llm.available and fingerprints:
                    llm_analysis = await self.llm.analyze_json(
                        "You are a web security analyst. Analyze these HTTP headers and identify the technology stack.",
                        f"Headers: {fingerprints}",
                    )
                    if llm_analysis:
                        asset_inventory[-1]["llm_analysis"] = llm_analysis
        finally:
            await client.close()

        return AgentResult(
            agent_name=self.name, status=AgentStatus.COMPLETED,
            outputs={"findings": findings, "asset_inventory": asset_inventory, "routes": routes},
